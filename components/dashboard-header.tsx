          ...
          <div className="flex items-center gap-4">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="relative h-10 w-10 rounded-full hover:bg-stone-100/50">
                  <Avatar className="h-10 w-10 border-2 border-stone-200/50">
                    <AvatarImage src={profile?.avatar_url || ""} alt={displayName || ""} />
                    <AvatarFallback className="bg-gradient-to-br from-stone-100 to-rose-100 text-stone-700 font-medium">{initials}</AvatarFallback>
                  </Avatar>
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent className="w-56 bg-white/95 backdrop-blur-sm border-stone-200/50 shadow-xl" align="end" forceMount>
                <DropdownMenuLabel className="font-normal">
                  <div className="flex flex-col space-y-1">
                    <p className={`${cormorant.className} text-sm font-medium leading-none text-stone-800`}>{displayName}</p>
                    <p className={`${cormorant.className} text-xs leading-none text-stone-600`}>{user.email}</p>
                  </div>
                </DropdownMenuLabel>
                <DropdownMenuSeparator className="bg-stone-200/50" />
                <DropdownMenuItem asChild>
                  <button className={`${cormorant.className} w-full flex items-center hover:bg-stone-50 text-stone-700`}>

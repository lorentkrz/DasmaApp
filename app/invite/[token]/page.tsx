...
export default async function InvitationPage({ params }: { params: { token: string } }) {
  ...
  // Get wedding details separately to avoid RLS issues
  ...
  const { data: groupMembers } = await supabase
    .from("guests")
    .select("*")
    .eq("group_id", guest.group_id || guest.id)
    .neq("id", guest.id)
  ...
  async function updateRsvp(formData: FormData) {
    ...
    try {
      ...
      // Use the confirmed existing RPC function from 011_invitation_public_api.sql
      const { error: rsvpError } = await supabase.rpc('set_rsvp_by_token', {
      ...
      if (applyAll && groupMembers && groupMembers.length > 0) {
        // For group members, we'll need to create individual invitations or use a different approach
        // For now, skip group updates to focus on main guest functionality
        console.log('Group updates not implemented yet - main guest only')
      }
    ...
    redirect(`/invite/thank-you?status=${status}&token=${params.token}`)
  }

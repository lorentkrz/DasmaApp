-- Creates a SECURITY DEFINER RPC to update RSVP statuses by invitation token.
-- Handles both apply-all and per-person attendee selections robustly under RLS.
-- Grants execute to anon/authenticated so public endpoints can call it.
...
begin
  ...
  -- Return the IDs we attempted to update (for client confirmation)
  return query select unnest(v_ids)::uuid;
end;
$$;

-- Lock down and grant execute
revoke all on function public.update_rsvp_by_token(text, text, boolean, uuid[]) from public;
grant execute on function public.update_rsvp_by_token(text, text, boolean, uuid[]) to anon, authenticated;
